...
feature_set = "smaller_" #this gives the prefix of the file your features are in 
features =np.load(feature_set + 'cell_featurevectors_DL.npy') #load in your features from the saved matrix file
names = np.load(feature_set + 'cell_filenames.npy') #load in the associated names in the saved matrix file
total_transfers = 24 #the total number of transfers you want to train/test on (training+test set)
...
for hold_x_out in [3,5,1]: #the number of images to hold out for each test (defines the training set)
    newfile = feature_set + '_100_MLP_cell_results_L' + str(hold_x_out) + 'O.csv' #where results will be printed
